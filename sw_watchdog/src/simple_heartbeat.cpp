...
void print_usage()
{
  std::cout << "Usage: simple_heartbeat [-h] --ros-args -p period:=value [...]\n\n"
    "required arguments:\n"
    "\tperiod: Period in positive integer milliseconds of the heartbeat "
    "signal.\n"
    "optional arguments:\n"
...
{
/**
 * A class that publishes heartbeats at a fixed frequency with the header set to
 * current time.
 */
...
public:
  ...
  : Node(
      "simple_heartbeat",
      options.start_parameter_event_publisher(false).start_parameter_services(false))
  {
    declare_parameter("period", 200);
    ...
    // The granted lease is essentially infite here, i.e., only reader/watchdog
    // will notify violations. XXX causes segfault for cyclone dds, hence pass
    // explicit lease life > heartbeat.
    ...
    timer_ =
      this->create_wall_timer(heartbeat_period, std::bind(&SimpleHeartbeat::timer_callback, this));
  }
