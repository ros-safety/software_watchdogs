...
void print_usage()
{
  std::cout << "Usage: windowed_watchdog lease max-misses [" << OPTION_AUTO_START <<
    "] [-h]\n\n"
    "required arguments:\n"
    "\tlease: Lease in positive integer milliseconds granted to the "
    "watched entity.\n"
    "\tmax-misses: The maximum number of lease violations granted "
    "to the watched entity.\n"
    "optional arguments:\n"
    "\t" <<
    OPTION_AUTO_START <<
    ": Start the watchdog on creation.  Defaults to false.\n"
    "\t" <<
    OPTION_PUB_STATUS <<
    ": Publish lease expiration of the watched entity.  "
    "Defaults to false.\n"
...
/**
 * Internally relies on the QoS deadline and liveliness policies provided by the
 * rmw implementation (e.g., DDS). The lease passed to this watchdog has to be >
 * the period of the heartbeat signal to account for network transmission times.
 */
...
public:
  ...
  : rclcpp_lifecycle::LifecycleNode("windowed_watchdog", options),
    autostart_(false),
    enable_pub_(false),
    topic_name_(DEFAULT_TOPIC_NAME),
    lease_misses_(0),
    qos_profile_(10)
  {
    ...
    if (args.size() < 3 || rcutils_cli_option_exist(&cargs[0], &cargs[0] + cargs.size(), "-h")) {
      print_usage();
      // TODO(robbel): Update the rclcpp_components template to be able to
      // handle exceptions. Raise one here, so stack unwinding happens
      // gracefully.
      std::exit(0);
    }
  ...
  void publish_status(uint16_t misses)
  {
    ...
    if (!status_pub_->is_activated()) {
      ...
      RCLCPP_INFO(
        get_logger(), "Publishing lease expiry (missed count: %u) at [%f]", msg->missed_number,
        now.seconds());
    }
    ...
    const rclcpp_lifecycle::State &)
  {
    ...
    heartbeat_sub_options_.event_callbacks.deadline_callback =
      [this](rclcpp::QOSDeadlineRequestedInfo & event) -> void {
        ...
        lease_misses_.fetch_add(
          static_cast<uint16_t>(event.total_count_change), std::memory_order_relaxed);

        publish_status(lease_misses_);
        // Transition lifecycle to deactivated state
        if (lease_misses_ >= max_misses_) {deactivate();}
      };

    // Catch the case where monitored entity disappears from the network
    // entirely (deadline QoS does not account for that)
    ...
    if (enable_pub_) {
      status_pub_ =
        create_publisher<sw_watchdog_msgs::msg::Status>("status", 1); /* QoS history_depth */
    }
    ...
    const rclcpp_lifecycle::State &)
  {
    ...
    if (enable_pub_) {status_pub_->on_activate();}
  ...
  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn on_deactivate(
    const rclcpp_lifecycle::State &)
  {
    heartbeat_sub_.reset();  // XXX there does not seem to be a 'deactivate' for
                             // subscribers.
    heartbeat_sub_ = nullptr;

    // Starting from this point, all messages are no longer sent to the network.
    if (enable_pub_) {status_pub_->on_deactivate();}
...
private:
  ...
  // activated to publish.
  std::shared_ptr<rclcpp_lifecycle::LifecyclePublisher<sw_watchdog_msgs::msg::Status>> status_pub_ =
    nullptr;
  /// Whether to enable the watchdog on startup. Otherwise, lifecycle
  /// transitions have to be raised.
  bool autostart_;
